#!/usr/bin/env python3
"""
Stardew Valley Focus Flow Dashboard Generator
...
class ASCIIRenderer:
    ...
    def sparkline(values, width=None):
        """
        Generate ASCII sparkline chart using simple ASCII characters.
        Example: __.--*^^**
        """
        if not values:
            return ''

        # Use only ASCII characters for guaranteed single-width
        chars = '_.-=*^'
        ...
        for v in values:
            if max_val == min_val:
                ...
                norm = int(((v - min_val) / (max_val - min_val)) * 5)
...
class DashboardGenerator:
    ...
    def render_ascii_dashboard(self, state):
        ...
        lines.append(r.box_line("STARDEW VALLEY FOCUS FLOW DASHBOARD", align='center'))
    ...
    def render_html(self, state, output_filename='dashboard.html'):

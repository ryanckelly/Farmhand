...
2. **FIRST: Regenerate HTML files for Railway deployment**:
   ```bash
   ...
   - Run this FIRST to ensure HTML is ready before terminal display

3. **SELECT TOP 5 ACTIVE UNLOCKS** (do this BEFORE asking user permission):
   - Read `save_snapshot.json` to get `unlockables_status`
...
4. **Generate and display the colored terminal dashboard**:
   ```bash
...
5. **ASK USER FOR PERMISSION**: After displaying the dashboard, ask "Would you like me to proceed to generate recommendations for your session today?"
   - If user says NO or wants to stop, end here (they may just want a quick status check)
   - If user says YES, continue to step 6

6. Read core files to understand context (only if user grants permission):
...
7. **VALIDATE recommendations using a subagent** (see CLAUDE.md for details):
...
8. Provide customized session strategy including:
